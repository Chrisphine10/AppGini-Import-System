...
function contacts_insert(){
	...
	$data['entity'] = makeSafe($_REQUEST['entity']);
		if($data['entity'] == empty_lookup_value){ $data['entity'] = ''; }
	...
	if($data['name']== ''){
		echo StyleSheet() . "\n\n<div class=\"alert alert-danger\">" . $Translation['error:'] . " 'Company': " . $Translation['field not null'] . '<br><br>';
	...
	sql('insert into `contacts` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . ', `entity`=' . (($data['entity'] !== '' && $data['entity'] !== NULL) ? "'{$data['entity']}'" : 'NULL') . ', `email`=' . (($data['email'] !== '' && $data['email'] !== NULL) ? "'{$data['email']}'" : 'NULL') . ', `phone`=' . (($data['phone'] !== '' && $data['phone'] !== NULL) ? "'{$data['phone']}'" : 'NULL') . ', `mobile`=' . (($data['mobile'] !== '' && $data['mobile'] !== NULL) ? "'{$data['mobile']}'" : 'NULL') . ', `contact_person`=' . (($data['contact_person'] !== '' && $data['contact_person'] !== NULL) ? "'{$data['contact_person']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
	...
	sql("insert ignore into membership_userrecords set tableName='contacts', pkValue='" . makeSafe($recID, false) . "', memberID='" . makeSafe(getLoggedMemberID(), false) . "', dateAdded='" . time() . "', dateUpdated='" . time() . "', groupID='" . getLoggedGroupID() . "'", $eo);
...
function contacts_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	// child table: shipping_db
	$res = sql("select `id` from `contacts` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `shipping_db` where `supplier`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
		$RetMsg = str_replace("<TableName>", "shipping_db", $RetMsg);
		return $RetMsg;
	...
	$res = sql("select `id` from `contacts` where `id`='$selected_id'", $eo);
	$id = db_fetch_row($res);
	$rires = sql("select count(1) from `shipping_db` where `shipping`='".addslashes($id[0])."'", $eo);
	$rirow = db_fetch_row($rires);
	if($rirow[0] && !$AllowDeleteOfParents && !$skipChecks){
		$RetMsg = $Translation["couldn't delete"];
		$RetMsg = str_replace("<RelatedRecords>", $rirow[0], $RetMsg);
...
function contacts_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `contacts` set       `name`=' . (($data['name'] !== '' && $data['name'] !== NULL) ? "'{$data['name']}'" : 'NULL') . ', `entity`=' . (($data['entity'] !== '' && $data['entity'] !== NULL) ? "'{$data['entity']}'" : 'NULL') . ', `email`=' . (($data['email'] !== '' && $data['email'] !== NULL) ? "'{$data['email']}'" : 'NULL') . ', `phone`=' . (($data['phone'] !== '' && $data['phone'] !== NULL) ? "'{$data['phone']}'" : 'NULL') . ', `mobile`=' . (($data['mobile'] !== '' && $data['mobile'] !== NULL) ? "'{$data['mobile']}'" : 'NULL') . ', `contact_person`=' . (($data['contact_person'] !== '' && $data['contact_person'] !== NULL) ? "'{$data['contact_person']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function contacts_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	$combo_entity->ListType = 0;
	$combo_entity->MultipleSeparator = ', ';
	$combo_entity->ListBoxHeight = 10;
	$combo_entity->RadiosPerLine = 1;
	if(is_file(dirname(__FILE__).'/hooks/contacts.entity.csv')){
		$entity_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/contacts.entity.csv')));
		$combo_entity->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions($entity_data)));
		$combo_entity->ListData = $combo_entity->ListItem;
	}else{
		$combo_entity->ListItem = explode('||', entitiesToUTF8(convertLegacyOptions("Supplier;;Shipping Agent")));
		$combo_entity->ListData = $combo_entity->ListItem;
	}
	...
	if($selected_id){
		...
		$combo_entity->SelectedText = ( $_REQUEST['FilterField'][1]=='3' && $_REQUEST['FilterOperator'][1]=='<=>' ? (get_magic_quotes_gpc() ? stripslashes($_REQUEST['FilterValue'][1]) : $_REQUEST['FilterValue'][1]) : "");
	}
	$combo_entity->Render();
	...
	if($selected_id){
		...
		$templateCode=str_replace('<%%VALUE(entity)%%>', html_attr($row['entity']), $templateCode);
	...
	$templateCode .= loadView('contacts-ajax-cache', $cache_data);
