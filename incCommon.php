	...
	*/
	...
	function getTableList($skip_authentication = false){
		...
		$arrTables = array(   
			...
			'shipping_db' => array('Imports Database', '', 'table.gif', 'None')
		);
	...
	function get_sql_fields($table_name){
		$sql_fields = array(   
			'contacts' => "`contacts`.`id` as 'id', `contacts`.`name` as 'name', `contacts`.`entity` as 'entity', `contacts`.`email` as 'email', `contacts`.`phone` as 'phone', `contacts`.`mobile` as 'mobile', `contacts`.`contact_person` as 'contact_person'",
			'shipping_db' => "`shipping_db`.`id` as 'id', IF(    CHAR_LENGTH(`contacts1`.`name`), CONCAT_WS('',   `contacts1`.`name`), '') as 'supplier', `shipping_db`.`description` as 'description', IF(    CHAR_LENGTH(`contacts2`.`name`), CONCAT_WS('',   `contacts2`.`name`), '') as 'shipping', if(`shipping_db`.`date_ordered`,date_format(`shipping_db`.`date_ordered`,'%d/%m/%Y'),'') as 'date_ordered', if(`shipping_db`.`bol_date`,date_format(`shipping_db`.`bol_date`,'%d/%m/%Y'),'') as 'bol_date', `shipping_db`.`bol_date_status` as 'bol_date_status', `shipping_db`.`designation` as 'designation', `shipping_db`.`status` as 'status', `shipping_db`.`unit` as 'unit', `shipping_db`.`quantity` as 'quantity', `shipping_db`.`currency` as 'currency', CONCAT('$', FORMAT(`shipping_db`.`unit_price`, 2)) as 'unit_price', CONCAT('$', FORMAT(`shipping_db`.`total_price`, 2)) as 'total_price', `shipping_db`.`payment_terms` as 'payment_terms', `shipping_db`.`due` as 'due', `shipping_db`.`profoma_invoice` as 'profoma_invoice', `shipping_db`.`copy_invoice` as 'copy_invoice', `shipping_db`.`commercial_invoice` as 'commercial_invoice', `shipping_db`.`bill_of_ladding` as 'bill_of_ladding', `shipping_db`.`clearing_document` as 'clearing_document', `shipping_db`.`other_documents` as 'other_documents', `shipping_db`.`logged_by` as 'logged_by', `shipping_db`.`bol_status` as 'bol_status'"
		);
	...
	function get_sql_from($table_name, $skip_permissions = false){
		$sql_from = array(   
			'contacts' => "`contacts` ",
			'shipping_db' => "`shipping_db` LEFT JOIN `contacts` as contacts1 ON `contacts1`.`id`=`shipping_db`.`supplier` LEFT JOIN `contacts` as contacts2 ON `contacts2`.`id`=`shipping_db`.`shipping` "
		);
	...
	function logInMember(){
		...
		if($_POST['signIn'] != ''){
			if($_POST['username'] != '' && $_POST['password'] != ''){
				...
				if(sqlValue("select count(1) from membership_users where lcase(memberID)='$username' and passMD5='$password' and isApproved=1 and isBanned=0")==1){
					...
					if($_POST['rememberMe']==1){
						@setcookie('imports_system_rememberMe', md5($username.$password), time()+86400*30);
					}else{
						@setcookie('imports_system_rememberMe', '', time()-86400*30);
					}
			...
			exit;
		}elseif((!$_SESSION['memberID'] || $_SESSION['memberID']==$adminConfig['anonymousMember']) && $_COOKIE['imports_system_rememberMe']!=''){
			$chk=makeSafe($_COOKIE['imports_system_rememberMe']);
	...
	function htmlUserBar(){
		...
		<nav class="navbar navbar-default navbar-fixed-top hidden-print" role="navigation">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?php echo PREPEND_PATH; ?>index.php"><i class="glyphicon glyphicon-home"></i> imports system</a>
			</div>
